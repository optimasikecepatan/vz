...
function glideHandleHeight(t) {
    ...
    let t = document.getElementsByClassName("loadmore");
const loadMorePosts = () => {
  ...
  loadMoreButton.addEventListener("click", async (event) => {
    ...
    const baseUrl = nextLink.href.split('?')[0];
    ...
    if (articles.length > 0) {
      ...
      // Update the next link href
      nextLink.href = pageUrl;

      // If no more articles or reached max pages, hide the button
      if (articles.length < parseInt(searchParams.get('result') || 12)) {
        loadMoreButton.style.display = 'none';
      }
    } else {
      loadMoreButton.style.display = 'none';
