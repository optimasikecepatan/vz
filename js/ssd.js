...
(function() {
    ...
    if (!postsContainer || !nextLink) return;

    // Parse URL and get initial page number
    const nextUrl = nextLink.getAttribute('href');
    const urlParams = new URLSearchParams(new URL(nextUrl).search);
    let currentPage = parseInt(urlParams.get('page')) || 1;
    
    // Get total pages from a data attribute (add this to your HTML)
    const totalPages = parseInt(loadMoreButton.dataset.totalPages) || 5; // Fallback to 5 if not set
    ...
    loadMoreButton.addEventListener('click', async function(e) {
        ...
        try {
            ...
            const nextPageUrl = `${baseUrl}?page=${currentPage}&result=12`;
            ...
            newPosts.forEach(post => {
                if (postsContainer.classList.contains('posts-block')) {
                    ...
                    posts_colc.append(post); // Make sure posts_colc is defined
                }
            });
